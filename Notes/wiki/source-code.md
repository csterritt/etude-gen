...
Repository for the `etude_params` aggregate, encapsulating the physical Drizzle columns behind a domain `EtudeParams` interface. `loadOrCreateEtudeParams(db, userId)` atomically inserts a default aggregate or loads the existing one — the caller that loses the insert race handles the UNIQUE-constraint violation on the owner reference as a load of the winner's aggregate, not as an error (the violation is detected across both the D1 and bun-sqlite drivers, which surface it with different error shapes). `loadEtudeParams(db, userId)` is an owner-scoped read returning `null` when none exists. `updateEtudeSetup(db, userId, expectedEpoch, values)` conditionally updates the setup-step fields (measure count, time signature, hand, key signature) using a compare-and-set on `aggregateEpoch`: the function first loads the current aggregate to compare the submitted values against the stored ones — when every submitted value is identical, the request is a no-op (no version increment, no write, no flag changes), avoiding spurious version bumps from a double submission of the same form. Otherwise it performs a single conditional Drizzle `update` with a `where` clause matching both `userId` and `aggregateEpoch === expectedEpoch`, so a request whose captured epoch no longer matches the stored value updates zero rows and returns `Result.err`. On success the same committed transition increments `workflowVersion` by 1, sets `setupConfirmed` to true, and updates the measure/meter/hand/key columns. When the submitted key differs from the stored key, the same transition also clears `notesConfirmed` and `splitConfirmed` to false (Issue 11 dependency map row for Key clears pitch selection and split boundary); when the key is identical but another field changed, those flags are left untouched. All three functions follow the `withRetry`/`toResult` Result-returning pattern from `src/lib/db-access.ts`. The default aggregate carries the PRD's practical defaults (8 measures, 4/4, C major, octave range 4, right hand), `workflowVersion` 1, `aggregateEpoch` 1, and all three step-confirmation flags (`setupConfirmed`, `notesConfirmed`, `splitConfirmed`) false. Exports `EtudeParams`, `loadOrCreateEtudeParams`, `loadEtudeParams`, `updateEtudeSetup`.

### setup-validator.ts

Authoritative domain validator for the setup step's four parameters: measure count (4-32 inclusive integer), time signature (one of 2/4, 3/4, 4/4), hand (left, right, both), and key (one of the eighteen supported keys). Lives in the Music Domain module so the route never trusts submitted values. `validateSetup(input)` narrows untrusted `unknown` values from the form parser and returns `Result<ValidSetup, SetupValidationFailure[]>` — multiple invalid fields are reported together so a student can correct them in one round. Invalid values are never silently coerced into plausible defaults: an empty string, null, undefined, a wrong type, or an out-of-range value is a rejection. The key field is delegated to `validateKey` from `src/lib/key-domain.ts`. Exports `MEASURE_MIN`, `MEASURE_MAX`, `SUPPORTED_METERS`, `SUPPORTED_HANDS`, `ValidSetup`, `SetupInput`, `SetupValidationFailure`, `validateSetup`.
