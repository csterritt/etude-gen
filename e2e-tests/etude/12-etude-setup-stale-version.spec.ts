...
test.describe('POST /etude/setup stale-version rejection (two-tab scenario)', () => {
  test(
    'a stale submission from a second tab is rejected and shows the newly current saved state',
    testWithDatabase(async ({ page }) => {
      ...
      const tabB = page.context().pages()[0] ? await page.context().newPage() : await page.context().newPage()
      ...
      // Tab A: submit a valid change (measures 16, meter 3/4, hands both).
      ...
      // Tab B: submit a different change carrying the stale version 1.
      // This must be rejected with a 303 redirect to /etude/setup.
      // Include the key field so the submission passes parsing/validation
      // and reaches the version CAS check (the actual behavior under test).
      const response = await postSetupViaBrowser(tabB, {
        measures: '12',
        meter: '4/4',
        hands: 'right',
        key: 'C major',
      ...
      // Tab B: navigate to /etude/setup and confirm the redisplayed form
      // shows the newly current saved state (Tab A's values), NOT Tab B's
      // submitted values.
      await tabB.goto(`${SERVER_BASE_URL}${ETUDE_SETUP_PATH}`)
      ...
      // The version field shows the current version (2), not the stale one.
