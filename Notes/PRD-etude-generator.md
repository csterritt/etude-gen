...
- Every mutable form carries the current workflow version. A compare-and-set update rejects stale versions and displays the newly current state with an explanatory error. Operation forms that submit no student values — generate, rendering retry, PDF, and Start Over — carry the same version as a precondition that is checked but not incremented, alongside their own lock and epoch guards.
- Changing setup selections clears all dependent note, duration, and split completion state. Review completion is derived from step confirmations and current value validity rather than persisted, so no review flag is written or cleared and `GET /etude/review` changes no state. Any retained Piece becomes stale and its score and PDF controls are hidden.
...
- New-Piece generation and PDF generation each have two distinct controls: an in-flight lock that prevents concurrent work and a last-success timestamp that enforces a post-success cooldown. The generation/render lock and the PDF lock are separate locks with separate owner identifiers, acquisition times, and expiry, so holding one never blocks acquiring the other and releasing one never affects the other.
- The aggregate also carries a monotonic epoch. Start Over increments it and account deletion moves it to a terminal value. Every conditional commit made by an operation request requires the epoch captured at lock acquisition to still be current, so a request that was calling LilyPond or writing storage while the workflow was cleared or the account deleted cannot publish a Piece, render state, cooldown, grant, or lock release. An artifact already written by such a request is unreachable and is cleaned up with `cleanupReason` `commit_failed`. The workflow version alone is insufficient here because Start Over resets parameters to defaults.
...
- PDF is stored in private R2 only for the grant lifecycle. The download GET authenticates the session, loads the owner-scoped grant, reads and validates the bounded object into response-owned bytes, atomically claims and consumes the grant, builds the attachment from those owned bytes, and only then initiates object cleanup. Claiming after a successful bounded read means a storage failure cannot spend the single use, and the response never streams from an object that is being deleted. One use means one successful server response attempt, not confirmed client receipt.
...
- Artifact deletion makes one initial attempt followed by three retries delayed 100, 200, and 400 milliseconds. Exhaustion emits a structured `artifact_cleanup_exhausted` log with `artifactId`, `artifactKind` (`svg` or `pdf`), `cleanupReason` (`replacement`, `start_over`, `grant_consumed`, `grant_expired`, `account_deleted`, or `commit_failed`), `attemptCount`, `lastErrorCategory`, `correlationId`, and `occurredAt`. The opaque artifact identifier contains no user identifier; the user operation proceeds and the object remains unreachable.
