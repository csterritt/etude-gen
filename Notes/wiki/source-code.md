...
Repository for the `etude_params` aggregate, encapsulating the physical Drizzle columns behind a domain `EtudeParams` interface. `loadOrCreateEtudeParams(db, userId)` atomically inserts a default aggregate or loads the existing one — the caller that loses the insert race handles the UNIQUE-constraint violation on the owner reference as a load of the winner's aggregate, not as an error (the violation is detected across both the D1 and bun-sqlite drivers, which surface it with different error shapes). `loadEtudeParams(db, userId)` is an owner-scoped read returning `null` when none exists. `updateEtudeSetup(db, userId, expectedEpoch, values)` conditionally updates the setup-step fields (measure count, time signature, hand) using a compare-and-set on `aggregateEpoch`: the `where` clause matches both `userId` and `aggregateEpoch === expectedEpoch`, so a request whose captured epoch no longer matches the stored value updates zero rows and returns `Result.err`. On success the same committed transition increments `workflowVersion` by 1, sets `setupConfirmed` to true, and updates the measure/meter/hand columns. All three functions follow the `withRetry`/`toResult` Result-returning pattern from `src/lib/db-access.ts`. The default aggregate carries the PRD's practical defaults (8 measures, 4/4, C major, octave range 4, right hand), `workflowVersion` 1, `aggregateEpoch` 1, and all three step-confirmation flags (`setupConfirmed`, `notesConfirmed`, `splitConfirmed`) false. Exports `EtudeParams`, `loadOrCreateEtudeParams`, `loadEtudeParams`, `updateEtudeSetup`.

### setup-validator.ts

Authoritative domain validator for the setup step's three parameters: measure count (4-32 inclusive integer), time signature (one of 2/4, 3/4, 4/4), and hand (left, right, both). Lives in the Music Domain module so the route never trusts submitted values. `validateSetup(input)` narrows untrusted `unknown` values from the form parser and returns `Result<ValidSetup, SetupValidationFailure[]>` — multiple invalid fields are reported together so a student can correct them in one round. Invalid values are never silently coerced into plausible defaults: an empty string, null, undefined, a wrong type, or an out-of-range value is a rejection. Exports `MEASURE_MIN`, `MEASURE_MAX`, `SUPPORTED_METERS`, `SUPPORTED_HANDS`, `ValidSetup`, `SetupInput`, `SetupValidationFailure`, `validateSetup`.

### etude-form-parser.ts

Reusable parameter-form parser, not specific to the setup form. Extracts typed raw values from a `FormData` submission, tolerating the hostile shapes of cross-cutting contract section 2 rule 5: an absent field, an empty string, a repeated field (multi-value), an unexpected extra field, and fields in an arbitrary order each resolve to a deterministic accept or field-addressable reject. None of them produces a thrown error, and none is silently coerced. Each caller supplies a `FieldSpec` declaring the expected field names, their target types, and an optional repeated-field policy (`reject` default, or `first-wins`). The setup form declares no normalization, so a repeated field is a reject. Designed for reuse by Issues 6, 7, 13, 14, and 16. Exports `RepeatedFieldPolicy`, `FieldSpecEntry`, `FieldSpec`, `ParseFailure`, `RawValues`, `parseParameterForm`.
