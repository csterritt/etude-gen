...
2. **"Current pitch" state after a repeated/mirrored bar is not defined.** Repeated bars copy pitches exactly; mirrored bars copy only rhythm/rests and generate fresh pitches. The PRD does not say what pitch the *next* fresh bar transitions from. Use the last pitched event of the just-completed bar (copied or freshly generated), but it must be stated, including across a trailing rest run.
