...
## Notes step: pitch selection (Issue 13)

The notes step is the second step of the etude workflow, reached when setup is confirmed but notes are not. The canonical-route resolver now redirects to `/etude/notes` when `setupConfirmed && !notesConfirmed` (cross-cutting contract section 5: the notes step is one coherent prerequisite — both pitches and durations must be confirmed for it to count as complete). This issue implements the pitch half; the duration half is Issue 14.

The pitch-selection form (`src/routes/build-etude-notes.tsx`) derives the available pitches from the stored key and octave range via `deriveAvailablePitches` and `parseStoredOctaves` (shared with the setup route from `src/lib/music-domain.ts`). The available pitches are presented as checkboxes, one per pitch, with accessible labels and `data-testid="pitch-field-<pitch>"`.

**First-derivation semantics** (`resolvePitchSelectionState` in `src/lib/pitch-selection-validator.ts`): when no pitch selection is stored (`selectedPitches` is null or empty), all available pitches are preselected — this is the "first derivation." Once a selection is stored, later renders show exactly the stored selection (including a narrowed one) and never re-expand it. After an Issue 11 clear (which sets `selectedPitches` to null when the key or octave range changes), the next render is again a first derivation with the full new available set preselected.

**Cardinality rules** (`validatePitchSelection`): one-hand mode (`'left'` or `'right'`) requires at least one pitch; two-hand mode (`'both'`) requires at least two pitches, with the exact field-level message "Select at least two pitches when using both hands." A submitted pitch not in the derived available set is rejected with a failure naming the unavailable pitch. The validator tolerates hostile shapes (a non-array, duplicates, reordered values, an empty array) deterministically without throwing.

**Select all** is a distinct submit button (`name="action" value="select-all"`) that ignores the submitted checkboxes, persists all available pitches via `updateEtudePitches`, and redirects 303 to `/etude/notes`. It works without client-side scripting (it is a server-side round-trip) and can never produce a cardinality error because it selects every available pitch.

**Persistence** (`updateEtudePitches` in `src/lib/etude-params-repository.ts`): conditionally sets `selectedPitches` and increments `workflowVersion` via a compare-and-set write matching the `updateEtudeSetup` CAS pattern (load → identical-resubmit short-circuit → conditional update with epoch + version guard → zero-row disambiguation). Does NOT set `notesConfirmed` — the notes step is confirmed only when both pitches and durations are confirmed (durations are Issue 14). Does NOT modify `selectedDurations`, `splitBoundary`, `setupConfirmed`, `notesConfirmed`, or `splitConfirmed`.

Stale-version and epoch-mismatch rejections redirect 303 with an explanatory error, redisplaying the committed saved state (not the submitted values). A rejected pitch submission redisplays the submitted pitches checked alongside the field-level cardinality error, with the error summary focused and linking into the pitch group.

## Notes step: duration selection (Issue 14)

This issue completes the notes step's duration half, making the step coherent: the notes step is confirmed, and later steps become reachable, only when pitches (Issue 13) and durations are both confirmed. The duration selection derives its offerability, eligibility, and corrective guidance from the Issue 12 rhythm catalog.

**Offerable-per-meter semantics** (`computeOfferableDurations` in `src/lib/duration-selection-validator.ts`): a duration is offerable for a meter when at least one catalog pattern for that meter contains it. For example, whole and dotted-half notes never fit a 2/4 measure, so 2/4 offers only half, quarter, dotted-quarter, and eighth. `resolveDurationSelectionState` implements the same first-derivation semantics as pitches: a freshly derived notes step preselects every individually compatible duration; a stored narrowed selection is never re-expanded; after an Issue 11 meter-change clear (which sets `selectedDurations` to null), the next render is again a first derivation with the new offerable set preselected.

**Eligibility validation** (`validateDurationSelection` in `src/lib/duration-selection-validator.ts`): a submitted duration set is valid only when at least one eligible complete-measure pattern exists — a pattern whose every token is in the selected set (via `computeEligibleRhythms`). Duplicates are de-duplicated and accepted; the stored set is normalized to the canonical order (`CANONICAL_DURATION_ORDER`: `W, H, D, Q, R, E`) regardless of submission order; an empty selection is rejected with "Select at least one duration."; an unknown token, and a supported token not offerable for the current meter, are rejected with a field-addressable error (never silently dropped). A set with no eligible pattern is rejected with the stable corrective message: it names the duration group, states that no complete measure can be built for the current meter, and names the smallest set of additional offered durations that would restore eligibility (`computeCorrectiveSuggestion`), exposed only by display label (e.g. "the eighth duration" or "the half and quarter durations") — never catalog patterns, internal token letters, or line numbers. This is the authoritative, no-script server path (user story 20).

**Combined persistence** (`updateEtudeNotes` in `src/lib/etude-params-repository.ts`): the ordinary Save on the notes form validates both pitches and durations and persists both halves in one compare-and-set write (load → identical-resubmit short-circuit → conditional update with epoch + version guard → zero-row disambiguation), setting `notesConfirmed: true`. It does NOT modify split state or setup. Select all remains pitch-only: it persists all available pitches via `updateEtudePitches` and never touches durations or `notesConfirmed`, so the step stays incomplete until a full combined save. Stale-version and epoch-mismatch rejections redirect 303 with an explanatory error showing the newly current saved state; a rejected duration submission redisplays the submitted selection checked alongside the group-level corrective error, with the error summary focused and linked into the duration group.

## Read-only step summaries and Back links (Issue 17)

Every later step of the etude workflow renders a read-only summary of the prior answers so the student can see what they have already chosen without being able to edit it from a later step. The summary is a shared presentational component, not per-route logic:

- **Model** (`src/lib/etude-summary.ts`): `deriveStepSummary(params, step)` is a pure function that reads the owner's committed aggregate snapshot and returns a `StepSummary` carrying exactly the fields confirmed at or before the requested step level (`'notes'`, `'split'`, or `'review'`). A `'notes'` summary carries only the setup fields; a `'split'` summary adds the notes fields (pitches, durations); a `'review'` summary adds the split fields (boundary, left/right hand pitch sets) when a boundary applies. Fields beyond the requested level are absent from the model (not null) so the component omits them from the DOM entirely. The function never re-derives available pitches, offerable durations, or eligible boundaries — those derivations belong to the form routes. It is pure: no DB, no side effects, no mutation, no throws.
- **Component** (`src/components/etude-summary.tsx`): `EtudeSummary` renders a `<section data-testid="etude-summary">` with a `<dl>` of labeled values, one per summary field. Each value is rendered as text only — no `input`, `select`, `textarea`, or `button` elements — so the summary is never editable. Pitch lists are comma-separated phrases; duration tokens are rendered via their `DURATION_LABELS` display labels (never raw tokens); the octave range is rendered as `min` or `min–max`. The component reveals no internal identifiers (no aggregate id, no workflow version, no epoch).
- **Back links**: each later step carries a canonical GET Back link as a plain anchor (`<a>`), not a form submit. The notes step links back to `/etude/setup`; the split step links back to `/etude/notes`; the review step links back to `/etude/split` (two-hand) or `/etude/notes` (one-hand). Following a Back link issues a GET to the prior step, discarding any unsaved edits on the current page — the student returns to the prior step's form with its current saved state, not the current page's unsaved edits.

The summary and Back link are wired into the notes route (`src/routes/build-etude-notes.tsx`), the split route (`src/routes/build-etude-split.tsx`), and the stub review route (`src/routes/build-etude-review.tsx`). The stub review route renders only the summary and Back link and defines no POST handler; Issue 19 will replace it with the full review step (Generate form, etc.). A direct GET to `/etude/review` when review is not reachable redirects to the canonical route resolved by `resolveCanonicalRoute` (cross-cutting contract section 5).

## Prerequisite redirects to the earliest incomplete step (Issue 18)

When a student directly requests a step that is not yet reachable — by typing the URL, using a stale bookmark, or reloading after an upstream change — the workflow redirects them to the earliest incomplete step with a safe explanatory message. This wires the cross-cutting contract section 5 redirect contract into every step route.

**Workflow service** (`src/lib/workflow-service.ts`): `computeCanonicalRoute(params)` wraps `resolveCanonicalRoute` (the ordering resolver from Issue 16) with value-validation for the stored downstream selections. When the ordering resolver returns `/etude/split` or `/etude/review`, the service additionally validates the stored pitches against the available set derived from the stored key and octaves, the stored durations against the offerable set for the stored meter, and the stored boundary against the eligible boundaries derived from the stored pitches. If any stored value no longer validates, the service returns the earliest step whose stored values are invalid (or the earliest unconfirmed step, whichever comes first). This catches corrupt states that the flag-only ordering resolver misses — e.g. a stored pitch removed from the available set by an upstream change, or a stored boundary no longer eligible after a pitch change. `isStepReachable(params, stepPath)` returns true when a step's prerequisites are met — the canonical route is at or after the requested step in the step ordering. This lets a student revisit a completed step to edit it (e.g., following a Back link from the review step to the split step) without being redirected forward. The split step is never reachable for one-hand workflows (it is skipped). The service also exports `PREREQUISITE_REDIRECT_MESSAGE`, a safe explanatory message that exposes no internal state or identifiers (no ids, version numbers, epoch values, pitch names, or boundary ids) per cross-cutting contract section 1 rule 6.

**Route guards**: the notes, split, and review routes each call `isStepReachable(params, stepPath)` and redirect with `redirectWithPrerequisiteMessage` (a new helper in `src/lib/redirects.tsx` that sets a dedicated `PREREQUISITE_REDIRECT_FOUND` cookie) when the step's prerequisites are not met — redirecting to `computeCanonicalRoute(params)`. A completed step remains visitable for editing (the student can follow a Back link to it), because `isStepReachable` returns true when the canonical route is at or after the requested step. The entry route (`GET /etude`) calls `computeCanonicalRoute` directly and uses the ordinary `redirectWithMessage` with an empty message — the entry point is not a prerequisite redirect, so no message is shown. The layout (`src/routes/build-layout.tsx`) renders the prerequisite-redirect message in a dedicated alert with `data-testid="prerequisite-redirect-message"`, distinct from the ordinary success and error alerts, so tests can distinguish a prerequisite redirect from a normal page load.

**Side effects in the redirect branch**: the split route's one-hand corrective clear (`clearEtudeSplit`, no version increment) and corrupt-state recovery (`clearEtudeSplit` with `unconfirmNotes: true`) are triggered from the redirect branch — they are corrective clears, not ordering decisions, and they run only when the split step is not the canonical route. This consolidates the previously separate one-hand and corrupt-state guards into the single canonical-route guard.
